<div class="mx-auto" dir="rtl">   
    ...
    <div class="mt-4">
        <div class="grid grid-cols-2 md:grid-cols-1 lg:grid-cols-3 gap-2">
            ...
            <div wire:click="addToCart({{$product->id}})" class="relative bg-white border border-gray-300 rounded overflow-hidden text-right {{ $counter > 1 ? 'hidden lg:block' : '' }}">
                ...
                <p class="text-gray-600 p-2 text-sm font-medium">{{$product->name}} (المنبقي: {{$product->quantity}})</p>
                <p class="text-green-600 p-2 pt-0 text-md font-bold">{{$currency_symbol . $product->price}}</p>
                
                @if($product->quantity < 1)
                <div class="absolute top-1 left-1 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded">
                    غير متوفر
                </div>
                @endif
            </div> 
