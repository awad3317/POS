<div>
    ...
    <div class="fixed inset-0 flex items-center justify-center z-50 bg-gray-600 bg-opacity-50 transition-opacity duration-500 ease-in-out" 
        style="display: {{ $error ? 'flex' : 'none' }}; opacity: {{ $error ? '1' : '0' }};" dir="rtl">
        <div wire:click.outside="$set('error', false)" 
            ...
            <div class="text-center">
                <h2 class="text-xl font-bold mb-4 text-red-600">تنبيه!</h2>
                <p class="text-gray-700 mb-4 font-medium">{{ $error }}</p>
                <button wire:click="$set('error', false)" class="px-6 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors">
                    إغلاق
                </button>
            </div>
        </div>
